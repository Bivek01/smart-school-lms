...
export default function TeacherSidebar({ isMobileOpen, setIsMobileOpen }) {
  ...
  const navItems = [
    { id: 'overview', label: 'Overview', path: '/teacher/dashboard/overview', icon: LayoutDashboard },
    { id: 'subjects', label: 'Subjects & Content', path: '/teacher/dashboard/subjects', icon: BookOpen },
    { id: 'homework', label: 'Homework & Grading', path: '/teacher/dashboard/homework', icon: ClipboardList },
    { id: 'attendance', label: 'Mark Attendance', path: '/teacher/dashboard/attendance', icon: CalendarCheck },
    { id: 'performance', label: 'Student Performance', path: '/teacher/dashboard/performance', icon: Award },
  ];
        ...
        <div>
          ...
          <div className="mx-4 my-6 p-3.5 rounded-2xl bg-slate-800/80 border border-slate-700/60 flex items-center space-x-3">
            ...
            <div className="overflow-hidden">
              <h4 className="text-sm font-semibold text-white truncate">{user?.name || 'Teacher'}</h4>
              <p className="text-xs text-slate-400 truncate">{user?.email || 'teacher@school.edu'}</p>
            </div>
          </div>
